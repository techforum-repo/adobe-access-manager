...
def test_favorites_and_recent_requests(tmp_path: Path, monkeypatch) -> None:
    ...
    database.initialize()
    database.replace_favorite_groups("admin@example.com", ["Group B", "Group A", "Group A"])
    assert database.list_favorite_groups("admin@example.com") == ["Group A", "Group B"]

    request_id = database.save_recent_request(
        "admin@example.com",
        [{"email": "user@example.com", "include": True}],
        ["Group A"],
        "Preview",
        {"assignments": 1},
    )
